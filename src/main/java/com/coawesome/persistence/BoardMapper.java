...
//          "SELECT DISTINCT user.name, board.board_id, board.public_level, board.catagory, board.likes_num, board.tag1, board.tag2, board.tag3," +
//          "board.line1_x, board.line1_y,board.line1, board.line2, board.line2_x, board.line2_y, board.created, board_image.stored_file_name, board.favorite_num, reply.replier, reply.likes_num, reply.reply, favorite.user_id as favorite FROM board " +
//          "INNER JOIN friend on board.user_id = friend.friend_id " +
//          "INNER JOIN board_image on board.board_id = board_image.board_id " +
//          "INNER JOIN user on board.user_id = user.user_id" +
//          "LEFT OUTER JOIN (SELECT * FROM favorite WHERE user_id = #{user_id}) as favorite ON board.board_id = favorite.board_id " +
